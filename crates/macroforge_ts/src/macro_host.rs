...
use crate::host::{MacroConfig, MacroDispatcher, MacroRegistry, PatchCollector, derived};
...
type PackageRegistrar = fn(&MacroRegistry) -> crate::host::Result<()>;
...
fn register_packages(
    ...
    for pkg in crate::host::package_registry::registrars() {
        embedded_map.entry(pkg.module).or_insert(pkg.registrar);
    }

    let derived_modules = crate::host::derived::modules();
    ...
    for module in requested {
        ...
        if derived_set.contains(module) {
            crate::host::derived::register_module(module, registry)?;
            found = true;
        }
