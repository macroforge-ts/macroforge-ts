...
impl<T> ToCode for oxc::allocator::Box<'_, T>
where
    T: ToCode,
{
...
impl<T> ToCode for oxc::allocator::Vec<'_, T>
where
    T: ToCode,
{
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
...
impl ToCode for oxc::span::Span {
    fn to_code(&self, _: &Ctx) -> syn::Expr {
        parse_quote!(oxc::span::Span::default())
    }
}

impl ToCode for oxc::span::Atom<'_> {
    fn to_code(&self, _: &Ctx) -> syn::Expr {
        let s = self.as_str();
    ...
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        match self {
            Expression::BooleanLiteral(e) => {
                ...
                parse_quote!(oxc::ast::ast::Expression::BooleanLiteral(#inner))
            }
            Expression::NullLiteral(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::NullLiteral(#inner))
            }
            Expression::NumericLiteral(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::NumericLiteral(#inner))
            }
            Expression::StringLiteral(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::StringLiteral(#inner))
            }
            Expression::Identifier(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::Identifier(#inner))
            }
            Expression::BinaryExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::BinaryExpression(#inner))
            }
            Expression::CallExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::CallExpression(#inner))
            }
            Expression::ComputedMemberExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::ComputedMemberExpression(#inner))
            }
            Expression::StaticMemberExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::StaticMemberExpression(#inner))
            }
            Expression::PrivateFieldExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Expression::PrivateFieldExpression(#inner))
            }
...
impl ToCode for IdentifierReference<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        parse_quote! {
            oxc::ast::ast::IdentifierReference {
                span: oxc::span::Span::default(),
                name: #name,
...
impl ToCode for BinaryExpression<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        let operator =
            syn::parse_str::<syn::Path>(&format!("oxc::ast::ast::BinaryOperator::{}", operator))
                .unwrap();
        parse_quote! {
            oxc::ast::ast::BinaryExpression {
                span: oxc::span::Span::default(),
                left: #left,
                right: #right,
                operator: #operator,
    ...
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        match self {
            Argument::SpreadElement(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Argument::SpreadElement(#inner))
            }
            _ => {
    ...
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        match self {
            MemberExpression::ComputedMemberExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::MemberExpression::ComputedMemberExpression(#inner))
            }
            MemberExpression::StaticMemberExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::MemberExpression::StaticMemberExpression(#inner))
            }
            MemberExpression::PrivateFieldExpression(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::MemberExpression::PrivateFieldExpression(#inner))
            }
        }
    }
}
...
impl ToCode for ComputedMemberExpression<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        parse_quote! {
            oxc::ast::ast::ComputedMemberExpression {
                span: oxc::span::Span::default(),
                object: #object,
...
impl ToCode for StaticMemberExpression<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        parse_quote! {
            oxc::ast::ast::StaticMemberExpression {
                span: oxc::span::Span::default(),
                object: #object,
                property: #property,
                optional: false,
...
impl ToCode for PrivateFieldExpression<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        parse_quote! {
            oxc::ast::ast::PrivateFieldExpression {
                span: oxc::span::Span::default(),
                object: #object,
                field: #field,
                optional: false,
    ...
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        match self {
            Statement::ExpressionStatement(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Statement::ExpressionStatement(#inner))
            }
            Statement::BlockStatement(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Statement::BlockStatement(#inner))
            }
            Statement::ReturnStatement(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Statement::ReturnStatement(#inner))
            }
            Statement::IfStatement(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Statement::IfStatement(#inner))
            }
            Statement::VariableDeclaration(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::Statement::VariableDeclaration(#inner))
            }
...
impl ToCode for ExpressionStatement<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        let expression = self.expression.to_code(cx);
        parse_quote! {
            oxc::ast::ast::ExpressionStatement {
                span: oxc::span::Span::default(),
                expression: #expression,
            }
        }
...
impl ToCode for VariableDeclaration<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        let kind = syn::parse_str::<syn::Path>(&format!(
            "oxc::ast::ast::VariableDeclarationKind::{}",
            kind
        ))
        .unwrap();
            ...
            BindingPatternKind::BindingIdentifier(e) => {
                ...
                parse_quote! {
                    oxc::ast::ast::BindingPattern {
                        kind: oxc::ast::ast::BindingPatternKind::BindingIdentifier(#inner),
...
impl ToCode for TSTypeParameterInstantiation<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        parse_quote! {
            oxc::ast::ast::TSTypeParameterInstantiation {
                span: oxc::span::Span::default(),
                params: #params,
            }
        }
    ...
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        match self {
            TSType::TSAnyKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSAnyKeyword(#inner))
            }
            TSType::TSUnknownKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSUnknownKeyword(#inner))
            }
            TSType::TSNumberKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSNumberKeyword(#inner))
            }
            TSType::TSBooleanKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSBooleanKeyword(#inner))
            }
            TSType::TSStringKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSStringKeyword(#inner))
            }
            TSType::TSVoidKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSVoidKeyword(#inner))
            }
            TSType::TSNullKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSNullKeyword(#inner))
            }
            TSType::TSUndefinedKeyword(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSUndefinedKeyword(#inner))
            }
            TSType::TSTypeReference(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSType::TSTypeReference(#inner))
            }
...
impl ToCode for TSUndefinedKeyword {
    fn to_code(&self, _: &Ctx) -> syn::Expr {
        parse_quote! {
            oxc::ast::ast::TSUndefinedKeyword {
                span: oxc::span::Span::default(),
            }
        }
    }
    ...
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        match self {
            TSTypeName::IdentifierReference(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSTypeName::IdentifierReference(#inner))
            }
            TSTypeName::QualifiedName(e) => {
                let inner = e.to_code(cx);
                parse_quote!(oxc::ast::ast::TSTypeName::QualifiedName(#inner))
            }
        }
    }
}
...
impl ToCode for Program<'_> {
    fn to_code(&self, cx: &Ctx) -> syn::Expr {
        ...
        parse_quote! {
            oxc::ast::ast::Program {
                span: oxc::span::Span::default(),
                source_type: oxc::span::SourceType::default(),
                source_text: "",
