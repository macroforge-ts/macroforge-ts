...
function collectEnumDerives(): EnumDeriveResults {
  ...
  result.statusEquals = safe(() =>
    m.statusEquals(Status.Active, Status.Active) as boolean
  );
  ...
  result.priorityDebug = safe(() =>
    m.priorityToString(Priority.High) as string
  );
  result.priorityEquals = safe(() =>
    m.priorityEquals(Priority.Low, Priority.Low) as boolean
  );
  result.colorDebug = safe(() => m.colorToString(Color.Red) as string);
  result.colorEquals = safe(() =>
    m.colorEquals(Color.Blue, Color.Blue) as boolean
  );
...
function collectTypeAliasDerives(): TypeAliasDeriveResults {
  ...
  result.userProfileEquals = safe(() =>
    m.userProfileEquals(user, user) as boolean
  );
