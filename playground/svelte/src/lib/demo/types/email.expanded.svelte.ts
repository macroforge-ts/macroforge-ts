...
export function __serializeEmail(value: Email, ctx: SerializeContext): Record<string, unknown> {
...
export function fromObjectEmail(
    ...
    try {
        ...
        if (PendingRef.is(resultOrRef)) {
            return Result.err([
                { field: '_root', message: 'Email.fromObject: root cannot be a forward reference' }
            ]);
        }
        ctx.applyPatches();
        if (opts?.freeze) {
...
export function __deserializeEmail(value: any, ctx: DeserializeContext): Email | PendingRef {
...
export function hasShapeEmail(obj: unknown): boolean {
...
export function isEmail(obj: unknown): obj is Email {
    if (!hasShapeEmail(obj)) {
        return false;
    }
    const result = fromObjectEmail(obj);
    return Result.isOk(result);
}

/** Nested error structure matching the data shape */ export type ErrorsEmail = {
    _errors: Option<Array<string>>;
    canEmail: Option<Array<string>>;
    emailString: Option<Array<string>>;
}; /** Nested boolean structure for tracking touched/dirty fields */
export type TaintedEmail = {
    canEmail: Option<boolean>;
    emailString: Option<boolean>;
}; /** Type-safe field controllers for this form */
export interface FieldControllersEmail {
    readonly canEmail: FieldController<boolean>;
    readonly emailString: FieldController<string>;
} /** Gigaform instance containing reactive state and field controllers */
export interface GigaformEmail {
    readonly data: Email;
    readonly errors: ErrorsEmail;
    readonly tainted: TaintedEmail;
    readonly fields: FieldControllersEmail;
...
export function createFormEmail(overrides?: Partial<Email>): GigaformEmail {
    ...
    let tainted = $state<TaintedEmail>({ canEmail: Option.none(), emailString: Option.none() });
    const fields: FieldControllersEmail = {
        canEmail: {
...
export function fromFormDataEmail(
    formData: FormData
): Result<Email, Array<{ field: string; message: string }>> {
