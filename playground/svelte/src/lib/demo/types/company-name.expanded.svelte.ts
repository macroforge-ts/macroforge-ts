...
@param value - The value to serialize
@param ctx - The serialization context */ export function companyNameSerializeWithContext(
  value: CompanyName,
      ...
      return Result.err([
        {
          field: "_root",
          message:
            "CompanyName.deserialize: root cannot be a forward reference",
        },
      ]);
    }
    ctx.applyPatches();
...
/** @internal */ function companyNameDeserializeWithContext(
  value: any,
...
/** Nested error structure matching the data shape */ export type CompanyNameErrors =
  { _errors: Option<Array<string>>; companyName: Option<Array<string>> };
/** Nested boolean structure for tracking touched/dirty fields */ export type CompanyNameTainted =
  { companyName: Option<boolean> };
/** Type-safe field controllers for this form */ export interface CompanyNameFieldControllers {
  readonly companyName: FieldController<string>;
}
/** Gigaform instance containing reactive state and field controllers */ export interface CompanyNameGigaform {
...
/** Creates a new Gigaform instance with reactive state and field controllers. */ export function companyNameCreateForm(
  ...
  const fields: CompanyNameFieldControllers = {
    companyName: {
      ...
      validate: (): Array<string> => {
        const fieldErrors = companyNameValidateField(
          "companyName",
          data.companyName,
        );
...
/** Parses FormData and validates it, returning a Result with the parsed data or errors. Delegates validation to fromStringifiedJSON() from @derive(Deserialize). */ export function companyNameFromFormData(
  formData: FormData,
): Result<CompanyName, Array<{ field: string; message: string }>> {
