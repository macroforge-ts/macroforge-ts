...
export function __serializeLead(value: Lead, ctx: SerializeContext): Record<string, unknown> {
...
export function fromObjectLead(
    ...
    try {
        ...
        if (PendingRef.is(resultOrRef)) {
            return Result.err([
                { field: '_root', message: 'Lead.fromObject: root cannot be a forward reference' }
            ]);
        }
        ctx.applyPatches();
        if (opts?.freeze) {
...
export function __deserializeLead(value: any, ctx: DeserializeContext): Lead | PendingRef {
...
export function hasShapeLead(obj: unknown): boolean {
...
export function isLead(obj: unknown): obj is Lead {
    ...
    const result = fromObjectLead(obj);
    return Result.isOk(result);
}

/** Nested error structure matching the data shape */ export type ErrorsLead = {
...
export interface FieldControllersLead {
...
export interface GigaformLead {
    readonly data: Lead;
    readonly errors: ErrorsLead;
    readonly tainted: TaintedLead;
    readonly fields: FieldControllersLead;
...
export function createFormLead(overrides?: Partial<Lead>): GigaformLead {
    ...
    const fields: FieldControllersLead = {
        id: {
            path: ['id'] as const,
            name: 'id',
...
export function fromFormDataLead(
    formData: FormData
): Result<Lead, Array<{ field: string; message: string }>> {
