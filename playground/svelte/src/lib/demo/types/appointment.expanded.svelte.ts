...
@param value - The value to serialize
@param ctx - The serialization context */ export function appointmentSerializeWithContext(
  value: Appointment,
  ...
  result["begins"] = ((v: DateTime.DateTime) => DateTime.formatIso(v))(
    value.begins,
  );
  ...
  if (value.recurrenceRule !== null) {
    result["recurrenceRule"] = recurrenceRuleSerializeWithContext(
      ...
      return Result.err([
        {
          field: "_root",
          message:
            "Appointment.deserialize: root cannot be a forward reference",
        },
      ]);
    }
    ctx.applyPatches();
...
@param ctx - The deserialization context */ export function appointmentDeserializeWithContext(
  value: any,
  ...
  instance.begins = ((raw: unknown) =>
    DateTime.unsafeFromDate(new Date(raw as string)))(obj["begins"]);
  {
  ...
  }
  {
    ...
    if (__raw_recurrenceRule === null) {
      ...
      const __result = recurrenceRuleDeserializeWithContext(
        __raw_recurrenceRule,
        ctx,
      );
...
/** Nested boolean structure for tracking touched/dirty fields */ export type AppointmentTainted =
  {
    id: Option<boolean>;
...
/** Type-safe field controllers for this form */ export interface AppointmentFieldControllers {
...
/** Gigaform instance containing reactive state and field controllers */ export interface AppointmentGigaform {
...
/** Creates a new Gigaform instance with reactive state and field controllers. */ export function appointmentCreateForm(
  ...
  const fields: AppointmentFieldControllers = {
    ...
    employees: {
      ...
      validate: (): Array<string> => {
        const fieldErrors = appointmentValidateField(
          "employees",
          data.employees,
        );
        return fieldErrors.map(
  ...
  function validate(): Result<
    ...
    Array<{ field: string; message: string }>
  > {
    return appointmentFromObject(data);
  }
...
/** Parses FormData and validates it, returning a Result with the parsed data or errors. Delegates validation to fromStringifiedJSON() from @derive(Deserialize). */ export function appointmentFromFormData(
  formData: FormData,
): Result<Appointment, Array<{ field: string; message: string }>> {
  ...
  {
    // Collect nested object fields with prefix "begins."
  ...
  return appointmentFromStringifiedJSON(JSON.stringify(obj));
}
