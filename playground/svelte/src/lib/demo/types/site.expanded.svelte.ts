...
export function __serializeSite(value: Site, ctx: SerializeContext): Record<string, unknown> {
...
export function fromObjectSite(
    ...
    try {
        ...
        if (PendingRef.is(resultOrRef)) {
            return Result.err([
                { field: '_root', message: 'Site.fromObject: root cannot be a forward reference' }
            ]);
        }
        ctx.applyPatches();
        if (opts?.freeze) {
...
export function __deserializeSite(value: any, ctx: DeserializeContext): Site | PendingRef {
...
export function hasShapeSite(obj: unknown): boolean {
...
export function isSite(obj: unknown): obj is Site {
    ...
    const result = fromObjectSite(obj);
    return Result.isOk(result);
}

/** Nested error structure matching the data shape */ export type ErrorsSite = {
...
export interface FieldControllersSite {
...
export interface GigaformSite {
    readonly data: Site;
    readonly errors: ErrorsSite;
    readonly tainted: TaintedSite;
    readonly fields: FieldControllersSite;
...
export function createFormSite(overrides?: Partial<Site>): GigaformSite {
    ...
    const fields: FieldControllersSite = {
        id: {
            path: ['id'] as const,
            name: 'id',
...
export function fromFormDataSite(
    formData: FormData
): Result<Site, Array<{ field: string; message: string }>> {
