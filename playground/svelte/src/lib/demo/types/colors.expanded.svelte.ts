...
export function __serializeColors(value: Colors, ctx: SerializeContext): Record<string, unknown> {
...
export function fromObjectColors(
    ...
    try {
        ...
        if (PendingRef.is(resultOrRef)) {
            return Result.err([
                { field: '_root', message: 'Colors.fromObject: root cannot be a forward reference' }
            ]);
        }
        ctx.applyPatches();
        if (opts?.freeze) {
...
export function __deserializeColors(value: any, ctx: DeserializeContext): Colors | PendingRef {
...
export function hasShapeColors(obj: unknown): boolean {
...
export function isColors(obj: unknown): obj is Colors {
    if (!hasShapeColors(obj)) {
        return false;
    }
    const result = fromObjectColors(obj);
    return Result.isOk(result);
}

/** Nested error structure matching the data shape */ export type ErrorsColors = {
    _errors: Option<Array<string>>;
    main: Option<Array<string>>;
    hover: Option<Array<string>>;
    active: Option<Array<string>>;
}; /** Nested boolean structure for tracking touched/dirty fields */
export type TaintedColors = {
    main: Option<boolean>;
    hover: Option<boolean>;
    active: Option<boolean>;
}; /** Type-safe field controllers for this form */
export interface FieldControllersColors {
...
export interface GigaformColors {
    readonly data: Colors;
    readonly errors: ErrorsColors;
    readonly tainted: TaintedColors;
    readonly fields: FieldControllersColors;
    validate(): Result<Colors, Array<{ field: string; message: string }>>;
    reset(overrides?: Partial<Colors>): void;
} /** Creates a new Gigaform instance with reactive state and field controllers. */
export function createFormColors(overrides?: Partial<Colors>): GigaformColors {
    let data = $state({ ...defaultValueColors(), ...overrides });
    let errors = $state<ErrorsColors>({
        _errors: Option.none(),
        main: Option.none(),
        hover: Option.none(),
        active: Option.none()
    });
    let tainted = $state<TaintedColors>({
        main: Option.none(),
        hover: Option.none(),
        active: Option.none()
    });
    const fields: FieldControllersColors = {
        main: {
            path: ['main'] as const,
            name: 'main',
    ...
    function validate(): Result<Colors, Array<{ field: string; message: string }>> {
        return fromObjectColors(data);
    }
    function reset(newOverrides?: Partial<Colors>): void {
        data = { ...defaultValueColors(), ...newOverrides };
        errors = {
            _errors: Option.none(),
            main: Option.none(),
            hover: Option.none(),
...
export function fromFormDataColors(
    formData: FormData
): Result<Colors, Array<{ field: string; message: string }>> {
    ...
    return fromStringifiedJSONColors(JSON.stringify(obj));
}
