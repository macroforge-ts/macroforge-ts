...
export function __serializeRoute(value: Route, ctx: SerializeContext): Record<string, unknown> {
...
export function fromObjectRoute(
    ...
    try {
        ...
        if (PendingRef.is(resultOrRef)) {
            return Result.err([
                { field: '_root', message: 'Route.fromObject: root cannot be a forward reference' }
            ]);
        }
        ctx.applyPatches();
        if (opts?.freeze) {
...
export function __deserializeRoute(value: any, ctx: DeserializeContext): Route | PendingRef {
...
export function hasShapeRoute(obj: unknown): boolean {
...
export function isRoute(obj: unknown): obj is Route {
    if (!hasShapeRoute(obj)) {
        return false;
    }
    const result = fromObjectRoute(obj);
    return Result.isOk(result);
}

/** Nested error structure matching the data shape */ export type ErrorsRoute = {
...
export interface FieldControllersRoute {
...
export interface GigaformRoute {
    readonly data: Route;
    readonly errors: ErrorsRoute;
    readonly tainted: TaintedRoute;
    readonly fields: FieldControllersRoute;
...
export function createFormRoute(overrides?: Partial<Route>): GigaformRoute {
    ...
    let tainted = $state<TaintedRoute>({
        id: Option.none(),
        techs: Option.none(),
        active: Option.none(),
        name: Option.none(),
    ...
    const fields: FieldControllersRoute = {
        id: {
            path: ['id'] as const,
            name: 'id',
...
export function fromFormDataRoute(
    formData: FormData
): Result<Route, Array<{ field: string; message: string }>> {
