...
import { accountName__serialize } from "./account-name.svelte";
import { colors__serialize } from "./colors.svelte";
import { did__serialize } from "./did.svelte";
import { email__serialize } from "./email.svelte";
import { ordered__serialize } from "./ordered.svelte";
import { phoneNumber__serialize } from "./phone-number.svelte";
import { sector__serialize } from "./sector.svelte";
import { Result } from "macroforge/utils";
import { DeserializeContext } from "macroforge/serde";
import { DeserializeError } from "macroforge/serde";
import type { DeserializeOptions } from "macroforge/serde";
import { PendingRef } from "macroforge/serde";
import { accountName__deserialize } from "./account-name.svelte";
import { colors__deserialize } from "./colors.svelte";
import { email__deserialize } from "./email.svelte";
import { sector__deserialize } from "./sector.svelte";
...
export function accountToStringifiedJSON(value: Account): string {
  const ctx = SerializeContext.create();
  return JSON.stringify(account__serialize(value, ctx));
}
export function accountToObject(value: Account): Record<string, unknown> {
  const ctx = SerializeContext.create();
  return account__serialize(value, ctx);
}
export function account__serialize(
  value: Account,
  ...
  result["orders"] = value.orders.map((item) => ordered__serialize(item, ctx));
  result["activity"] = value.activity.map((item) => did__serialize(item, ctx));
  result["customFields"] = value.customFields;
  result["accountName"] = accountName__serialize(value.accountName, ctx);
  result["sector"] = sector__serialize(value.sector, ctx);
  result["memo"] = value.memo;
  result["phones"] = value.phones.map((item) =>
    phoneNumber__serialize(item, ctx),
  );
  result["email"] = email__serialize(value.email, ctx);
  result["leadSource"] = value.leadSource;
  result["colors"] = colors__serialize(value.colors, ctx);
...
export function accountFromStringifiedJSON(
  json: string,
  opts?: DeserializeOptions,
): Result<Account, Array<{ field: string; message: string }>> {
  try {
    const raw = JSON.parse(json);
    return accountFromObject(raw, opts);
  } catch (e) {
...
export function accountFromObject(
  ...
  try {
    ...
    const resultOrRef = account__deserialize(obj, ctx);
...
export function account__deserialize(
  value: any,
  ...
  }
  {
    ...
    {
      const __result = accountName__deserialize(__raw_accountName, ctx);
  ...
  }
  {
    ...
    {
      const __result = sector__deserialize(__raw_sector, ctx);
  ...
  }
  {
    ...
    {
      const __result = email__deserialize(__raw_email, ctx);
  ...
  }
  {
    ...
    {
      const __result = colors__deserialize(__raw_colors, ctx);
...
export function accountIs(obj: unknown): obj is Account {
  ...
  const result = accountFromObject(obj);
...
/** Nested boolean structure for tracking touched/dirty fields */ export type AccountTainted =
  {
    id: Option<boolean>;
...
/** Type-safe field controllers for this form */ export interface AccountFieldControllers {
...
/** Gigaform instance containing reactive state and field controllers */ export interface AccountGigaform {
...
/** Creates a new Gigaform instance with reactive state and field controllers. */ export function accountCreateForm(
  ...
  const fields: AccountFieldControllers = {
    ...
    customFields: {
      ...
      validate: (): Array<string> => {
        const fieldErrors = accountValidateField(
          "customFields",
          data.customFields,
        );
        return fieldErrors.map(
...
/** Parses FormData and validates it, returning a Result with the parsed data or errors. Delegates validation to fromStringifiedJSON() from @derive(Deserialize). */ export function accountFromFormData(
  formData: FormData,
): Result<Account, Array<{ field: string; message: string }>> {
  ...
  return accountFromStringifiedJSON(JSON.stringify(obj));
}

export const Account = {
  defaultValue: accountDefaultValue,
  toStringifiedJSON: accountToStringifiedJSON,
  toObject: accountToObject,
  __serialize: account__serialize,
  fromStringifiedJSON: accountFromStringifiedJSON,
  fromObject: accountFromObject,
  __deserialize: account__deserialize,
