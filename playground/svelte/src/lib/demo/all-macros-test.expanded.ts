...
export namespace SvelteAllMacrosTest {export function toJSON(self: SvelteAllMacrosTest): Record<string, unknown> {const result: Record<string, unknown> = {}; result["id" ]= self.id; result["title" ]= self.title; result["content" ]= self.content; result["apiKey" ]= self.apiKey; result["count" ]= self.count; result["enabled" ]= self.enabled; return result;}}

export namespace SvelteAllMacrosTest {export function is(data: unknown): data is SvelteAllMacrosTest {if(typeof data!== "object" || data === null || Array.isArray(data)){return false;}const obj = data as Record<string, unknown>; if(!("id" in obj)|| typeof obj["id" ]!== "string" ){return false;}if(!("title" in obj)|| typeof obj["title" ]!== "string" ){return false;}if(!("content" in obj)|| typeof obj["content" ]!== "string" ){return false;}if(!("apiKey" in obj)|| typeof obj["apiKey" ]!== "string" ){return false;}if(!("count" in obj)|| typeof obj["count" ]!== "number" ){return false;}if(!("enabled" in obj)|| typeof obj["enabled" ]!== "boolean" ){return false;}return true;}export function fromJSON(data: unknown): Result<SvelteAllMacrosTest, string[]> {if(!is(data)){return Result.err(["SvelteAllMacrosTest.fromJSON: type validation failed" ]);}const obj = data as Record<string, unknown>; const errors: string[]= []; if(errors.length> 0){return Result.err(errors);}return Result.ok(data); }}
