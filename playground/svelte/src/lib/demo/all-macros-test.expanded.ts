...
export function toStringSvelteAllMacrosTest(value: SvelteAllMacrosTest): string {
    const parts: string[] = [];
...
export function cloneSvelteAllMacrosTest(value: SvelteAllMacrosTest): SvelteAllMacrosTest {
    return {
        id: value.id,
        title: value.title,
...
export function equalsSvelteAllMacrosTest(a: SvelteAllMacrosTest, b: SvelteAllMacrosTest): boolean {
    if (a === b) return true;
    return (
        a.id === b.id &&
        a.title === b.title &&
...
export function toStringifiedJSONSvelteAllMacrosTest(value: SvelteAllMacrosTest): string {
    const ctx = SerializeContext.create();
    return JSON.stringify(__serializeSvelteAllMacrosTest(value, ctx));
}
export function toObjectSvelteAllMacrosTest(value: SvelteAllMacrosTest): Record<string, unknown> {
    const ctx = SerializeContext.create();
    return __serializeSvelteAllMacrosTest(value, ctx);
}
export function __serializeSvelteAllMacrosTest(
...
export function fromStringifiedJSONSvelteAllMacrosTest(
    json: string,
    opts?: DeserializeOptions
): Result<SvelteAllMacrosTest, Array<{ field: string; message: string }>> {
    try {
        const raw = JSON.parse(json);
        return fromObjectSvelteAllMacrosTest(raw, opts);
    } catch (e) {
...
export function fromObjectSvelteAllMacrosTest(
    obj: unknown,
    opts?: DeserializeOptions
): Result<SvelteAllMacrosTest, Array<{ field: string; message: string }>> {
    try {
        const ctx = DeserializeContext.create();
        const resultOrRef = __deserializeSvelteAllMacrosTest(obj, ctx);
        if (PendingRef.is(resultOrRef)) {
            return Result.err([
                {
                    field: '_root',
                    message: 'SvelteAllMacrosTest.fromObject: root cannot be a forward reference'
                }
            ]);
        }
        ctx.applyPatches();
...
export function __deserializeSvelteAllMacrosTest(
    value: any,
...
export function validateFieldSvelteAllMacrosTest<K extends keyof SvelteAllMacrosTest>(
    field: K,
    value: SvelteAllMacrosTest[K]
): Array<{ field: string; message: string }> {
    return [];
}
export function validateFieldsSvelteAllMacrosTest(
