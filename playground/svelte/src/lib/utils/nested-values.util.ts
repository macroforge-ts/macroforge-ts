...
type NormalizeDuration<T> = T extends Duration
  ? DurationEncoded | DurationTuple
  : T;
...
type IsCallable<T> = T extends (...args: Array<never>) => unknown
  ? true
  : false;
...
type SeenHas<Seen, T> = [T] extends [never]
  ? false
  : [T] extends [Seen]
    ? true
    : false;
...
type TupleKeys<P extends ReadonlyArray<unknown>> = {
  ...
  >]: I extends `${infer N extends number}` ? N : I extends number ? I : never;
}[Exclude<keyof P, keyof ReadonlyArray<unknown>>];
type ToReadonlyTuple<P extends ReadonlyArray<Key>> = {
  readonly [I in keyof P]: P[I];
};

type StopOnSeen<T, Seen, Fallback, Next> =
  SeenHas<Seen, T> extends true ? Fallback : Next;
 ...
 */
type ValidKeyAt<T, Seen = never> =
  Base<T> extends infer B
    ? [B] extends [Seen]
...
> = [D] extends [0]
  ...
  : P extends readonly []
    ...
    : P extends readonly [
          infer H extends Key,
          ...infer R extends ReadonlyArray<Key>,
        ]
...
> =
  NormalizeTarget<T> extends infer Base
    ? StopOnSeen<
      ...
      >
    : never;
export type ResolveDeepPath<_T, P, _Seen = never> =
  P extends ReadonlyArray<Key> ? P : never;
...
// ---- DeepValueUnion ----
export type DeepValueUnion<T, Seen = never> =
  NormalizeTarget<T> extends infer Base
    ? SeenHas<Seen, Base> extends true
      ...
      : Base extends ReadonlyArray<infer E>
        ...
        : Base extends Array<infer E>
          ...
          : IsNonFunctionRecord<Base> extends true
            ? {
                [K in keyof Base]-?: DeepValueUnion<
                  Base[K],
                  SeenAdd<Seen, Base>
                >;
              }[keyof Base]
...
// ---- DeepLeafValueUnion ----
export type DeepLeafValueUnion<T, Seen = never> =
  NormalizeTarget<T> extends infer Base
    ? SeenHas<Seen, Base> extends true
      ? never
      : Base extends ReadonlyArray<infer E>
        ...
        : Base extends Array<infer E>
          ...
          : IsNonFunctionRecord<Base> extends true
            ? {
                [K in keyof Base]-?: DeepLeafValueUnion<
                  Base[K],
                  SeenAdd<Seen, Base>
                >;
              }[keyof Base]
...
type DeepPathNonEmpty<T, D extends number = 5> =
  NormalizeTarget<T> extends infer Base
    ? Base extends unknown
...
type NormalizeDepth<
  ...
  Fallback extends number,
> = Depth extends keyof DepthMap ? Depth : Fallback;
...
> =
  Extract<DeepPathInternal<T>, ToReadonlyTuple<P>> extends never
...
type DeepLeafPathHelper<T, Seen = never> =
  NormalizeTarget<T> extends infer Base
    ? SeenHas<Seen, Base> extends true
      ? never
      : Base extends ReadonlyArray<infer E>
          ...
          : IsNonFunctionRecord<Base> extends true
            ? {
                ...
                > extends true
                  ...
                  : readonly [
                      K,
                      ...DeepLeafPathHelper<Base[K], SeenAdd<Seen, Base>>,
                    ];
              }[keyof Base]
            : readonly []
    : never;

export type DeepLeafPath<T> = (IsLeaf<T> extends true
  ? readonly []
  : DeepLeafPathHelper<T>) &
  DeepPathInternal<T>;
...
>(obj: T, path: P): DeepValue<T, P> | undefined {
...
>(obj: T, path: P, value: unknown): value is DeepValue<T, P> {
  if (!isResolvedDeepPath(obj, path)) {
