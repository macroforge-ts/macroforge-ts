...
pub fn generate(interface_name: &str, fields: &[ParsedField]) -> TsStream {
    ...
    ts_template! {
        {>> Parses FormData and validates it, returning a Result with the parsed data or errors. Delegates validation to fromJSON() from @derive(Deserialize). <<}
        export function fromFormData(formData: FormData): Result<Data, Errors> {
            const obj: Record<string, unknown> = {};

            @{field_extractions}

            // Delegate validation to the fromJSON generated by @derive(Deserialize)
            return @{interface_name}.fromJSON(obj);
        }
    }
}
