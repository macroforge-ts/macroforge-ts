...
The `| 0` (bitwise OR with zero) at the end of each step coerces the result to a 32-bit signed
integer, preventing floating-point drift from repeated multiplication. This is equivalent to casting
to `i32` in Rust and consistent with Java's `Objects.hash()` implementation.
