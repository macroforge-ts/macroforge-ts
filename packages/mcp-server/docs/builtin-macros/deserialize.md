...
standalone functions (`nameDeserialize`, `nameDeserializeWithContext`,
`nameIs`) that delegate to them:
...
`nameDeserialize`/`nameDeserializeWithContext`/`nameIs`; unlike the other
