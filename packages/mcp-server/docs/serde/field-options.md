...
`serializeWith` / `deserializeWith` hand a single field to your own function — the escape hatch for a shape the generated code can't express. For a type you use repeatedly across many classes, prefer [foreign types](/docs/serde/foreign-types), which configure the behavior once globally.
