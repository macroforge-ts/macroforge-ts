...
describe("CodeActionsProvider", function () {
  ...
  it("provide quickfix for adding jsDoc type to props", async () => {
    ...
    const addJsDoc = codeActions.find((fix) =>
      fix.title === "Infer type of 'abc' from usage"
    );
  ...
  it("provide quickfix for adding jsDoc type to non props when props exist", async () => {
    ...
    const addJsDoc = codeActions.find((fix) =>
      fix.title === "Infer type of 'ab' from usage"
    );
      ...
      diagnostics: [
        {
          ...
          message:
            "Cannot find name 'FixAllImported'. Did you mean 'FixAllImported3'?",
          range: range,
          source: "ts",
        },
      ],
