...
export interface DocumentSnapshot extends ts.IScriptSnapshot, DocumentMapper {
    ...
    /**
     * Diagnostics produced by macroforge macro expansion, if any.
     *
     * `start`/`end` are byte offsets into the text that was fed to
     * `expandSync` (i.e. the pre-expansion snapshot text), not into the
     * expanded text this snapshot exposes and not into the original document.
     * Consumers such as the DiagnosticsProvider currently apply them to the
     * post-expansion text, so positions may drift inside or after generated
     * regions.
     */
 ...
 * have no mapping back to the original document.
 */
export class SvelteDocumentSnapshot implements DocumentSnapshot {
    ...
    /**
     * Diagnostics from macroforge expansion. Offsets refer to the
     * pre-expansion svelte2tsx output, see {@link DocumentSnapshot.macroDiagnostics}.
     */
    public macroDiagnostics: MacroDiagnostic[] = [];

    /**
     * Runs macroforge expansion over the svelte2tsx output and replaces
     * `this.text` with the expanded code. The `tsxMap` is not updated, so
...
export class JSOrTSDocumentSnapshot extends IdentityMapper implements DocumentSnapshot {
    ...
    /**
     * Diagnostics from macroforge expansion. Offsets refer to the text
     * passed to expansion (post-kit augmentation, pre-expansion), see
     * {@link DocumentSnapshot.macroDiagnostics}.
     */
    public macroDiagnostics: MacroDiagnostic[] = [];

    /**
     * Runs macroforge expansion over `this.text` and replaces it with the
     * expanded code. Called at the end of `adjustText`, i.e. after SvelteKit
     * augmentation: for kit files the `addedCode` offsets were computed
     * against the pre-expansion text, so position mapping via
     * `getGeneratedPosition`/`getOriginalPosition` can drift when macro
