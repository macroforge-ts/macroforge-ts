...
export interface CompletionResolveInfo
  extends
    Pick<ts.CompletionEntry, "data" | "name" | "source">,
    TextDocumentIdentifier {
  position: Position;
...
export class CompletionsProviderImpl
  implements CompletionsProvider<CompletionResolveInfo> {
  constructor(
      ...
      if (clientSupportsItemsDefault) {
        ...
        completionList.items.forEach((item) => {
          item.commitCharacters ??=
            commitCharactersOptions.defaultCommitCharacters;
        });
      }
    }
  ...
  private getWordAtPosition(document: Document, offset: number) {
    ...
    return {
      ...
      range,
      defaultTextEditRange: wordRange.start === wordRange.end
        ? undefined
        : range,
    };
  }
  ...
  private findTypingsNamespaceSymbol(
    ...
    for (const part of rest) {
      ...
      symbol = typeChecker.getExportsOfModule(symbol).find((symbol) =>
        symbol.name === part
      );
    }
  ...
  async resolveCompletion(
    ...
    if (actions) {
      ...
      completionItem.additionalTextEdits =
        (completionItem.additionalTextEdits ?? []).concat(
          edit,
        );
    }
