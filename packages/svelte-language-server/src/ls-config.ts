...
export interface TsUserPreferencesConfig {
  ...
  importModuleSpecifierEnding:
    ts.UserPreferences["importModuleSpecifierEnding"];
  quoteStyle: ts.UserPreferences["quotePreference"];
  autoImportFileExcludePatterns:
    ts.UserPreferences["autoImportFileExcludePatterns"];
...
export class LSConfigManager {
  ...
  private _updateTsUserPreferences(
    ...
    this.tsUserPreferences[lang] = {
      ...
      includeCompletionsWithSnippetText:
        config.suggest?.includeCompletionsWithSnippetText ??
          true,
      ...
      excludeLibrarySymbolsInNavTo:
        config.workspaceSymbols?.excludeLibrarySymbols ?? true,
    };
  }
  ...
  async getFormatCodeSettingsForFile(
    ...
    return {
      ...
      baseIndentSize: prettierConfig.svelteIndentScriptAndStyle === false
        ? 0
        : indentSize,
      indentSize,
