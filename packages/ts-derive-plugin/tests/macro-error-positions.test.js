...
class User {
  ...
  // Macro diagnostics (ts-macros, ts-macros-generated) handle their own positions
  for (const diag of diagnostics) {
    if (diag.start !== undefined) {
      const isMacroDiag =
        diag.source === "ts-macros" || diag.source === "ts-macros-generated";
      assert(
  ...
  const generatedDiags = diagnostics.filter(
    (d) => d.source === "ts-macros-generated",
  );
...
a.toString();`;
  ...
  // ts-macros-generated diagnostics are allowed because they're at the specific macro name
  const nonMacroDiags = diagnostics.filter(
    (d) => d.source !== "ts-macros" && d.source !== "ts-macros-generated",
  );
...
const s = new Second();`;
  ...
  // ts-macros and ts-macros-generated diagnostics handle their own positions
  for (const diag of diagnostics) {
    if (diag.start !== undefined) {
