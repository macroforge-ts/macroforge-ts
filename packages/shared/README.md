...
- **`hasMacroAnnotations`** - Checks whether source code contains a macroforge JSDoc annotation — `@derive(...)`, `@cfg(...)`, `@deprecated`, `@mustUse`, or `@nonExhaustive`.
...
- **`MacroManifestEntry`** - One macro exported by an external macro package (from `__macroforgeGetManifest*`).
- **`DecoratorManifestEntry`** - One decorator exported by an external macro package.
- **`ExpandOptions`** - Options accepted by the native engine's `expandSync(code, filepath, options)`.
- **`MacroManifest`** - Aggregated manifest for an external macro package.
- **`ConfigLoadResult`** - Result from parsing a config file (as returned by the native `loadConfig`).
